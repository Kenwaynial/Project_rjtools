...
export default function MainPanel({ activeTab, vdResult, scResult, vdValues }) {
  if (activeTab === 'vdrop') {
    return (
      <main className="flex-1 bg-white border border-slate-200 rounded-xl p-6 flex flex-col gap-5 min-w-0 overflow-y-auto shadow-sm">
        <h2 className="text-lg font-bold text-slate-800 tracking-tight">Voltage Drop Result</h2>
          ...
          {vdResult ? (
            <motion.div key="vd-result" {...fadeUp} className="flex flex-col gap-4">
              <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                <div className="px-4 py-3 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
                  <span className="text-[0.65rem] font-bold text-slate-800 uppercase tracking-wider">Voltage Drop Summary Table</span>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-slate-200 text-slate-500 text-[0.55rem] uppercase tracking-wider font-semibold bg-slate-50">
                  ...
                  <tbody className="divide-y divide-slate-100 font-mono text-[0.7rem]">
                    <tr className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-4 py-2.5 font-sans text-xs text-slate-700">System Source Voltage</td>
                      <td className="px-4 py-2.5 text-slate-400">V_sys</td>
                      <td className="px-4 py-2.5 text-right text-slate-900">{vdValues.sysVoltage} V</td>
                    </tr>
                    <tr className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-4 py-2.5 font-sans text-xs text-slate-700">One-way Conductor Distance</td>
                      <td className="px-4 py-2.5 text-slate-400">D</td>
                      <td className="px-4 py-2.5 text-right text-slate-900">{vdValues.distance} m</td>
                    </tr>
                    <tr className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-4 py-2.5 font-sans text-xs text-slate-700">Line Load Current</td>
                      <td className="px-4 py-2.5 text-slate-400">I</td>
                      <td className="px-4 py-2.5 text-right text-slate-900">{vdValues.current} A</td>
                    </tr>
                    <tr className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-4 py-2.5 font-sans text-xs text-slate-700">Calculated Impedance per 305m</td>
                      <td className="px-4 py-2.5 text-slate-400">Z = √(R² + X²)</td>
                    ...
                    <tr className="bg-slate-50 transition-colors">
                      <td className="px-4 py-3 font-sans text-xs font-semibold text-slate-800">Percent Voltage Drop</td>
                      <td className="px-4 py-3 text-slate-400">V_% = (V_drop / V_sys) × 100</td>
            ...
            <motion.div key="vd-empty" {...fadeUp} className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-sm text-slate-400">Enter values and click Calculate</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
  ...
  return (
    <main className="flex-1 bg-white border border-slate-200 rounded-xl p-6 flex flex-col gap-5 min-w-0 overflow-y-auto shadow-sm">
      <h2 className="text-lg font-bold text-slate-800 tracking-tight">Short Circuit Result</h2>
...
function StepCard({ label, steps }) {
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
      <h4 className="text-xs font-semibold text-slate-800 uppercase tracking-wider mb-4 pb-2 border-b border-slate-100">{label}</h4>
      <div className="relative pl-4 border-l border-slate-200 space-y-4 ml-2.5">
        {steps.map((s, i) => {
          ...
          return (
            <div key={i} className="relative group">
              <div className="absolute -left-[27px] top-1 w-4 h-4 rounded-full bg-slate-50 border border-primary/30 text-primary flex items-center justify-center">
                <span className="text-[0.45rem] font-bold">{i + 1}</span>
              </div>
              <div className="bg-slate-50 border border-slate-200/80 rounded-lg p-3 hover:border-primary/20 transition-colors">
                <h5 className="text-[0.7rem] font-bold text-slate-800 tracking-wide">{title}</h5>
                <p className="text-[0.65rem] text-slate-500 leading-relaxed mt-1 mb-2">
                  {desc}
                </p>
